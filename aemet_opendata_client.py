...
__REQUEST_ENDPOINTS = {
    "inventory": "/api/valores/climatologicos/inventarioestaciones/todasestaciones/",
...
__COLUMNS_EVENTS = {"season	cateogory", "nam", "start", "end"}
...
def exists_inventory() -> bool:
    if not os.path.exists(get_file("inventory")):
        logging.debug(f"... inventario no encontrado.")
        return False
    logging.debug(f"... inventario disponible.")
    return True
...
def __request_inventory() -> pd.DataFrame:
    try:
        response = requests.get(
            __build_request_url("inventory"), headers=__REQUEST_HEADERS
        )
...
def __transform_inventory(df: pd.DataFrame) -> pd.DataFrame:
    stations_df = df.copy()
    stations_df.rename(columns=columns_on_stations(), inplace=True)
    stations_df[["province", "name"]] = stations_df[["province", "name"]].applymap(
        str.title
    )
    stations_df["latitude"] = stations_df["latitude"].apply(
        lambda x: __convert_dms_to_degrees(x, hemisphere=x[-1])
    )
    stations_df["longitude"] = stations_df["longitude"].apply(
        lambda x: __convert_dms_to_degrees(x, hemisphere=x[-1])
    )
    return stations_df[columns_on_stations]


def fetch_inventory():
    logging.info(f"Comprobando inventario de estaciones.")
    if not exists_inventory():
        logging.info("... descargando nuevos datos del inventario.")
        df = __request_inventory()
        logging.info("... preparando datos del inventario de estaciones.")
        df = __transform_inventory(df)
        try:
            logging.debug(f"... almacenando datos en {get_file('inventory')}.")
            df.to_csv(
                get_file("inventory"),
                index=False,
                sep="\t",
...
def fetch_warnings(event: str, start: datetime, end: datetime):
    ...
    if not exists_warnings(event):
        ...
        try:
            ...
            df = __merge_warnings_data(get_dir("warnings", event))
        except Exception as e:
        ...
        try:
            ...
            df = __transform_warnings_data(df)
        except Exception as e:
        ...
        try:
            ...
            df = __clean_warnings_data(df)
        except Exception as e:
            logging.error(f"Error al limpiar avisos: {e}")
            raise

        df = df[columns_on_warnings()]
...
def __merge_warnings_data(path: str) -> pd.DataFrame:
    ...
    df = pd.DataFrame(columns=__COLUMNS_CAP)
        ...
        for cap in cap_file_list:
            try:
                ...
                for param in selected_info.findall(".//cap:parameter", __NAMESPACES):
                    ...
                    cap_probability = parameters.get(
                        "AEMET-Meteoalerta probabilidad", None
                    )
                    if cap_probability:
                        cap_probability = cap_probability.replace("%", "")
                        if "mayor" in cap_probability:
                            cap_probability = [cap_probability, 100]
                        elif "menor" in cap_probability:
                            cap_probability = [0, cap_probability]
                        else:
                            cap_probability = cap_probability.split("-")
                    else:
                        cap_probability = [0, 100]
                ...
                if cap_severity in list(__WARNINGS_MAPPING.keys()):
                    df.loc[len(df)] = {
                        ...
                        "expires": cap_expires,
                        "min_probability": cap_probability[0],
                        "max_probability": cap_probability[1],
...
def __transform_warnings_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["min_probability"] = pd.to_numeric(df["min_probability"], errors="coerce")
    df["min_probability"] = df["min_probability"] / 100
    df["max_probability"] = pd.to_numeric(df["max_probability"], errors="coerce")
    df["max_probability"] = df["max_probability"] / 100
    ...
    optimized_df = pd.DataFrame(processed_rows)

    return optimized_df

def __clean_warnings_data(df: pd.DataFrame) -> pd.DataFrame:
    warning_df = df
...
def fetch_observations(
    ...
    if not exists_observations(event):
        df = pd.DataFrame(columns=list(__COLUMNS_OBSERVATIONS.keys()))
        ...
        df.rename(columns=__COLUMNS_OBSERVATIONS, inplace=True)
        df = df[columns_on_observations()]
        try:
            logging.info("... filtrando datos de observaciones.")
            df = __filter_observations_data(df, stations)
        except Exception as e:
        ...
        try:
            ...
            df = __transform_observations_data(df)
        except Exception as e:
        ...
        try:
            ...
            df = __clean_observations_data(df)
        except Exception as e:
...
def __filter_observations_data(df: pd.DataFrame, stations=[]) -> pd.DataFrame:
    observation_df = df
    observation_df = observation_df[observation_df["idema"].isin(stations)]
    return observation_df


def __transform_observations_data(df: pd.DataFrame) -> pd.DataFrame:
    observation_df = df
    observation_df["date"] = pd.to_datetime(observation_df["date"], format="%Y-%m-%d")
    observation_df["min_temperature"] = pd.to_numeric(
        observation_df["min_temperature"].str.replace(",", "."), errors="coerce"
    )
    observation_df["max_temperature"] = pd.to_numeric(
        observation_df["max_temperature"].str.replace(",", "."), errors="coerce"
    )
    observation_df["precipitation"] = pd.to_numeric(
        observation_df["precipitation"].str.replace(",", "."), errors="coerce"
    )
    observation_df["max_wind_speed"] = pd.to_numeric(
        observation_df["max_wind_speed"].str.replace(",", "."), errors="coerce"
    )
    observation_df.dropna(inplace=True)
    return observation_df

def __clean_observations_data(df: pd.DataFrame) -> pd.DataFrame:
    return df[columns_on_observations()]
