...
def ensure_directories(event: str, start: datetime, end: datetime) -> bool:
    ...
    """
    for d in constants.DIR_PATHS.values():
...
def __coordinate_dms_to_degrees(dms_coordinate: str, hemisphere: str) -> float:
    """
...
"""
    Warnings
"""
def exists_files_caps(event: str) -> bool:
    """
...
def __extract_data_caps(event: str) -> pd.DataFrame:
    """
    ...
    df = pd.DataFrame(columns=constants.columns_on_capxml())
...
def convert_caps_to_warnings(event: str) -> pd.DataFrame:
    """
    ...
    warnings = __extract_data_caps(event)
    ...
    warnings = __transform_raw_warnings(warnings)
    ...
    warnings = __clean_raw_warnings(warnings)
...
def __transform_raw_warnings(warnings: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    logging.info("Mapping severity to priority and dropping rows with NaN priority...")
    warnings["priority"] = warnings["severity"].map(constants.mapping_severity())
    warnings.dropna(subset=["priority"], inplace=True)
    ...
    warnings["geocode"] = pd.to_numeric(warnings["geocode"], downcast="integer", errors="coerce")
...
def __clean_raw_warnings(warnings: pd.DataFrame) -> pd.DataFrame:
    """
...
def __prepare_data_stations(stations: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    stations = stations.loc[:, constants.columns_on_stations()]
...
"""
    Events
"""
def retrieve_data_events() -> pd.DataFrame:
    """
...
"""
    Thresholds
"""
def retrieve_data_thresholds() -> pd.DataFrame:
    """
...
def __prepare_data_thresholds(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    try:
        cols = constants.columns_on_thresholds()
        for c in cols:
...
"""
    Observations
"""
def exists_data_observations(event: str) -> bool:
    """
...
def retrieve_data_observations(event: str, stations: list) -> pd.DataFrame:
    ...
    Raises:
        ...
        pd.errors.ParserError: If there is an error parsing the CSV file.
    """
    return __prepare_data_observations(pd.read_csv(constants.retrieve_filepath("observations", event=event), stations=stations), sep="\t")
...
def __prepare_data_observations(observations: pd.DataFrame, stations: list) -> pd.DataFrame:
    ...
    observations = observations[constants.columns_on_observations()]
    ...
    for column in ["min_temperature", "max_temperature", "precipitation", "max_wind_speed"]:
